            ...
            if len(playHand) != 0:
                ...
                while(True):
                    try:
                        ...
                        print("This integer is not accepted!")
                        time.sleep(1)
...
def cardReport(allHand, playHandId):
    ...
    for i in range(6):
        for j in range(len(allHand)):
            ...
            if (i == 2 and isInPlay) or (i == 3 and isInPlay == False):
                ...
                if allHand[j][1] == "Hearts":
                    string += Fore.RED + allHand[j][0]
                elif allHand[j][1] == "Diamonds":
                    string += Fore.YELLOW + allHand[j][0]
                elif allHand[j][1] == "Clubs":
                    string += Fore.GREEN + allHand[j][0]
                elif allHand[j][1] == "Spades":
                    string += Fore.BLUE + allHand[j][0]
                elif allHand[j][1] == "Stars":
                    string += Fore.RED + Back.YELLOW + allHand[j][0]
                elif allHand[j][1] == "Moons":
                    string += Fore.BLACK + allHand[j][0]
                elif allHand[j][1] == "Moles":
                    string += Fore.MAGENTA + allHand[j][0]
                elif allHand[j][1] == "Arenels":
                    string += Fore.MAGENTA + Back.BLACK + allHand[j][0]

                if allHand[j][0] == "10" or allHand[j][0] == "11" or allHand[j][0] == "12":
                    string += Fore.WHITE + Back.WHITE + "  "
                else:
                    string += Fore.WHITE + Back.WHITE + "   "
                string += Back.BLACK + " "   
    ...
    print(string)
    string = "\nActions:\n1-11 --- Select a Card by Index\nPLAY --- Play Selected Hand\nDISC --- Discard Selected Hand\n1-99(When hand types are shown) --- Select a Hand Type by Index\n"
    print(string)
