...
def round(num):
    ...
    while(BATTLE):
        report(chipsToWin, totChips, False, None, None, None, None)
        ...
        if action == "PLAY":
            if len(playHand) != 0:
                ...
                for i in range(len(playHand)):
                    ...
                    report(chipsToWin, totChips, True, playHand, i, nChips, nMult)
                    time.sleep(0.7)    # Need to show scoring
...
def report(chipsToWin, totChips, isPlayed, playHand, cardNum, nChips, nMult):
    global HANDS
    global DISCARDS
    global ANTE
    global ROUND
    ...
    if isPlayed:
        for i in range(len(playHand)):
            string += Fore.GREEN + Back.BLACK + "▄▄▄▄▄▄▄▄▄" + Fore.GREEN
    string += "▄\n  █                                            "
    if isPlayed:
        for i in range(len(playHand)):
            string += Fore.WHITE + Back.BLACK + "         " + Fore.GREEN
    string += "█\n  █    " + Fore.LIGHTRED_EX + "Ante: " + Fore.WHITE + str(ANTE) + Fore.LIGHTBLUE_EX + "   Round: " + Fore.WHITE + str(ROUND) + Fore.GREEN
    for i in range(24 - len(str(ROUND)) - len(str(ANTE))):
        string += " "
    if isPlayed:
        for i in range(len(playHand)):
            string += Fore.WHITE + Back.WHITE + "       " + Back.BLACK + "  " + Fore.GREEN
    string += "█\n  █    " + Fore.YELLOW + "Chips to win: " + Fore.WHITE + str(int(chipsToWin)) + Fore.GREEN
    for i in range(26 - len(str(int(chipsToWin)))):
        string += " "
    if isPlayed:
        for i in range(len(playHand)):
            string += Fore.WHITE + Back.WHITE + "       " + Back.BLACK + "  " + Fore.GREEN 
    string += "█\n  █    " + Fore.CYAN + "Current Chips: " + Fore.WHITE + str(totChips) + Fore.GREEN 
